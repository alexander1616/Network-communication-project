...
//Helper function to construct Selectfd
Selectfd_t*  makeSelect(Container_t *p){
	int nfds = p-> numElement;
	if (nfds == 0){
		printf("No elements in container.\n");
		return 0;
	...
	Selectfd_t *s;
	FDElement_t **np = (FDElement_t **)p->pool;
	s = calloc(1, sizeof(struct Selectfd_t));
	if (s == NULL){
...
void selectEvent(Container_t* p, int usec){
    ...
    while(1){
		...
		SelectStruct_t sfds;
		makeSelect(p, &sfds, sockfd, 60, 0);
        printf("\n\nGoing into select for connection: \n");
        retval = select(sfds.numFd, &sfds.x, NULL, NULL, &sfds.tv);
        if (retval < 0){
